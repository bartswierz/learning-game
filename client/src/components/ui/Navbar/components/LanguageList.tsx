      ...
      {languageList.map(({ id, language, flag }) => (
        <li key={id} style={{ width }}>
          <button
            ...
            className={`${
              activeLanguage === id ? `bg-${theme}-primary text-white` : "bg-slate-300"
            } px-4 py-2 hover:bg-${theme}-primary hover:text-white transition-colors duration-200 rounded-md cursor-pointer flex align-items w-full gap-2`}
          >
            {flag}
            {language}
          </button>
